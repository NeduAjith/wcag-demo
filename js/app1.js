...
function initKeyboardShortcuts() {
  document.addEventListener("keydown", (e) => {
    const shortcutEnabled = document.getElementById("shortcut-toggle");
    if (shortcutEnabled && shortcutEnabled.checked && e.key.toLowerCase() === "b") {
      const searchBox = document.getElementById("site-search");
      if (searchBox) searchBox.focus();
    }
  });
}
/* ---- Form Validation for Understandable ---- */
function validateForm() {
  const name = document.getElementById("name");
  const status = document.getElementById("form-status");
  if (!name.value.trim()) {
    name.setAttribute("aria-invalid", "true");
    status.textContent = "Error: Name is required. Please enter your full name.";
    return false;
  } else {
...
// Show popup automatically after 5 minutes
setTimeout(() => {
  openTimeoutPopup();
}, 300000); // 5 min = 300,000ms
...
function openTimeoutPopup() {
  ...
  // Move focus to the Extend Session button
...
function closeTimeoutPopup() {
  ...
  // Return focus to last focused element (before popup)
...
function extendSession() {
  ...
  // Restart the 5-minute countdown after extension
  setTimeout(() => {
    openTimeoutPopup();
  }, 300000);
}
...
/* ---- Accessible Modal (Good Example) ---- */
let lastFocusedTrigger = null;
let accessibleModalKeyHandler;
...
function badTrapHandler(e) {
  if (e.key === 'Tab') {
    e.preventDefault();
    document.querySelector('#trapped-modal button').focus();
  }
}
/* Understandable Form Validation Example */
...
/* ---- Robust Status Message ---- */
function showRobustStatus() {
  document.getElementById("robust-status").innerText = "Saved successfully!";
}
